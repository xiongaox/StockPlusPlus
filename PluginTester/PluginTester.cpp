...
{
    // 移除 F1 / 帮助跳转 GitHub Wiki，避免截图等全局快捷键冲突与误触
}
...
{
    // 【TEMP-PREVIEW】“方案A”（宿主托盘气泡）的形态预览：
    // 真机 TrafficMonitor 由其托盘图标发 Shell_NotifyIcon 气泡；
    // 测试器无常驻托盘图标，此处临时创建隐藏图标弹同款气泡，约5秒后自动回收。
    ...
    lstrcpynW(nid.szInfoTitle, L"股票阈值提醒预览", 64);
    lstrcpynW(nid.szInfo, strMsg, 256);
    lstrcpynW(nid.szTip, L"PluginTester 预览托盘图标（临时）", 128);
