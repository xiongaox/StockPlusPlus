...
public:
	...
	// 载荷 = ini 全文 + 台账 JSON 段（trades 表存于 stock_trades.db，不随 ini 走，故显式携带）
	...
	static bool ApplyBackupPayload(const std::string& payload, std::wstring& errorMsg,
		bool* ledgerRestored = nullptr);
...
private:
	...
	// 备份载荷中的台账段起止标记：ini 全文之后另起一行写入
	// kTradeLedgerBeginMarker + 单行 JSON + kTradeLedgerEndMarker，恢复时按标记切分
	static const char* kTradeLedgerBeginMarker;
	static const char* kTradeLedgerEndMarker;

	// 把 trades 表序列化为单行 JSON（无记录也算成功，返回 "[]"）
	static bool BuildTradeLedgerJson(std::string& jsonOut);
	// 拆分备份载荷：ini 内容写 configDataOut，台账段 JSON 写 ledgerJsonOut（无该段则 cleared 返回 false）
	static void SplitBackupPayload(const std::string& payload, std::string& configDataOut,
		std::string& ledgerJsonOut, bool& hasLedger);
	// 解析台账 JSON 并重建 trades 表；返回是否成功（空数组为合法的「清空台账」）
	static bool RestoreTradeLedger(const std::string& ledgerJson, std::wstring& errorMsg);
