...
{
	const int maxScroll = max(0, CalcPageContentHeight() - ScrollViewportHeight());
...
void CManagerDialog::UpdateControlsLayout()
{
	...
	// 「检查更新」按钮：关于页底部固定区右下角（不随日志滚动，其他页面隐藏）
	bool isAbout = (m_current_page == PAGE_ABOUT);
	if (m_about_update_btn.GetSafeHwnd())
	{
		m_about_update_btn.MoveWindow(CalcAboutUpdateBtnRect());
...
void CManagerDialog::OnPaint()
{
	...
	// 关于页例外：它自带三段式几何（顶部信息卡与底部操作区固定，只有日志视口内滚动），
	// 不参与这里的整体位移，否则固定的两段会被一起推走
	const bool aboutPage = (m_current_page == PAGE_ABOUT);
	CRect drawRect = contentRect;
	const bool scrolled = !aboutPage && ((m_page_scroll_y > 0) || (CalcPageContentHeight() > contentRect.Height()));
	if (scrolled)
	{
...
namespace
{
	...
	const wchar_t* kItems_0923[] = {
		L"•  【新增】 启动时自动按台账重算全部持仓：净持与含费摊薄成本随配置加载统一回填（此前只挂在台账增删改上，台账与持仓脱节的旧配置会一直错下去，需逐股手动触发）；云端备份恢复路径同样兜底重算，保证换机同步后台账与持仓必然一致，安全阀不变（台账无买入/清仓不改写）",
		L"•  【修复】 移除分时图左上角「模拟时间」徽标：该回测辅助默认开启且仅会话内生效，每次重启都会回到模拟 10:40 截断分时序列，干扰正常盯盘，现连同截断逻辑整体移除，界面回到无徽标状态",
		L"•  【优化】 分时全天时间轴切换按钮改为固定文案「全天」：此前按当前轴形态显示「全天/实时」两态文案，现与 MA/BL 等功能按钮风格统一（名字固定，点亮=全天轴开启），点击切换行为不变",
	};
	const wchar_t* kItems_0922[] = {
		L"•  【新增】 分时图支持「完整时间线」全天X轴：副图标题栏新增「全天/实时」切换按钮，全天模式X轴铺满当日全部交易时段（A股240/港股340/美股390 槽位），网格与时间标签按槽位时刻均匀分布（含午休跳变），数据未到右侧时右半轴保持空白网格并随时间同步推进；轴偏好持久化保存，悬浮窗重建后仍生效；分时视图左上角另增「模拟时间」徽标（会话内生效），点击可按模拟时刻截断分时序列，便于回放观察两种X轴的盘中切换效果",
		L"•  【优化】 接口检测页卡片第三行的「最近采样」状态改为右对齐，右缘与心跳条、可用率统计对齐；职责说明与采样文字超长时各自省略号截断互不重叠",
	...
	const wchar_t* kItems_0919b[] = {
		...
		L"•  【修复】 从行情中心点击领头股/主力股名跳到日K后，关掉重开悬浮窗再右键，行情中心会退回默认的「板块资金流」页而回不到刚才那一页（页签与跳转来源股票只在「隐藏暂存」时保留，从任务栏重开的换绑重建会丢）；现在两种重建方式都会保留浏览页签与临时视图来源股票，右键能回到跳转前那一页",
		L"•  【新增】 涨跌趋势页的涨跌分布柱现可点击，弹出该档成分股列表（名称/代码/现价/涨跌幅），点任意一行跳转该股日K线（右键返回行情中心）；涨停与跌停两档直接用已抓到的涨停/跌停池名单、点开即出，其余档按涨跌幅区间向服务端分页取名单、滚到接近末尾自动续取；浮层只绘制可见行，五千余只也不卡",
		L"•  【修复】 从行情中心点股票跳去日K后，右键返回要干等好几秒才回到原页面：分时缓存原来是逐点写库（一天 240 个点就是 240 次提交），中途把全局数据库锁攥住八九秒，而切换股票、从K线返回行情中心都要同步读一次缓存，只能干等这把锁；现改为整批一个事务写入（同类问题的基金净值缓存、分时成交明细一并修正），「删旧+写新」也随之变成原子操作，不会再有半截数据",
		L"•  【优化】 行情中心主力资金页的第三条曲线由「ETF净流入」改为「创业板净流入」：ETF 没有全市场聚合的分时资金流接口，那条线只能靠插件开着时本地逐分钟攒点，重启、换日都会清零，因此长期画不出来；创业板指是服务端直接下发整日 240 个点，与沪、深两条线同级可靠，重启即恢复。原「资金净申购」页的 ETF 排行不受影响",
		L"•  【修复】 修正 09-18 那次 ETF 曲线修复的遗留缺陷：采样只在 ETF 排行页被拉取时才触发，主力资金页从不请求该数据，导致曲线即便服务端可用也攒不到点；改用服务端数据后此依赖一并去除"
	};
	const wchar_t* kItems_0919[] = {
		L"•  【优化】 同日同方向的多次做T不再叠成一串方块：合并为一个 B/S 方块，右侧以小号数字标出笔数（如 B 后跟 4 表示当天买入 4 笔），价格区不再被占满；只做一笔时不显示数字",
		L"•  【修复】 日K视图的 B/S 标记改画在均线与布林带之上，不再被横穿的线条盖住（此前日K与分时的绘制层级不一致）",
		L"•  【修复】 分时图 B/S 标记改用「日期+分钟」匹配成交，修正当天所有成交被并到开盘第一个点上的问题；成交暂不可见时改用后续可见的一笔定位"
	};
	...
	const wchar_t* kItems_0916_v208[] = {
		...
		L"•  【新增】 K线族视图新增「区域」统计：拖动框选显示区间涨幅/最高/最低/振幅卡片，左右边界手柄可逐柱微调，选区锚定柱子不随缩放滚动丢失",
		L"•  【修复】 修复运行中偶发闪退：sqlite3 以 SQLITE_THREADSAFE=0 编译而数据库被界面与后台线程并发访问，现为数据库层全部方法加递归锁串行化",
		L"•  【新增】 涨跌趋势成交额升级东财市场概况同款口径：盘中新增「较前一日同期」同分钟放量/缩量差额与幅度，预测全天改用昨日同分钟进度法",
		L"•  【修复】 分组管理「状态栏显示」列因阈值提醒列插入右移导致点击勾选失效；勾选即时落盘，双击勾选列不再叠加弹窗",
		L"•  【优化】 持仓「成本价」列展示三位小数，与编辑弹窗口径一致",
		L"•  【新增】 每日涨跌幅阈值提醒：分组管理新增「阈值提醒」列，双击暗色弹窗设置涨幅/跌幅阈值，当日首次达标经宿主托盘气泡提醒一次，跨日自动重置",
		L"•  【新增】 悬浮窗销毁重建后完整恢复浏览状态；分时图买卖点绘制信号名称小字；气泡页新增最强/最弱板块卡片",
		L"•  【新增】 行情中心顶栏新增手动刷新按钮，无视新鲜度与失败退避强制重拉当前页数据",
		L"•  【修复】 昨收缺失时沿用最近一次有效昨收兜底涨跌幅与当日持仓收益；指数成交额与行情快照按交易日严格区分，修正较昨日全天口径与基金净值跨日错位"
	};
	...
	const AboutLogGroup kAboutLogGroups[] = {
		{ L"2026-09-23 (v2.0.13)", kItems_0923, _countof(kItems_0923) },
		{ L"2026-09-22 (v2.0.12)", kItems_0922, _countof(kItems_0922) },
...
// 关于页滚动内容自然总高：日志内容高 + 视口内上边距（顶部信息卡与底部固定区都不参与滚动）。
// 必须把上边距算进来：绘制起点是「视口顶 + 上边距 - 滚动量」，若只按内容高算 maxScroll，
...
void CManagerDialog::DrawAboutPage(Gdiplus::Graphics& g)
{
	...
	// ===== 更新日志：唯一滚动区（视口裁剪 + 负向滚动偏移） =====
	{
		...
		g.FillRectangle(&panelBg, logRf);
		g.DrawRectangle(&panelPen, logRf);

		Gdiplus::GraphicsState st = g.Save();
		g.SetClip(Gdiplus::RectF(static_cast<Gdiplus::REAL>(logViewport.left), static_cast<Gdiplus::REAL>(logViewport.top),
			static_cast<Gdiplus::REAL>(logViewport.Width()), static_cast<Gdiplus::REAL>(logViewport.Height())));
...
BOOL CManagerDialog::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
		...
		m_current_page == PAGE_ABOUT)
	{
		...
		if (maxScroll > 0 && scrollArea.PtInRect(clientPt))
		{
...
void CManagerDialog::OnSize(UINT nType, int cx, int cy)
{
	...
	if (m_search_dropdown.GetSafeHwnd())
		m_search_dropdown.HidePopup();
	// 窗口尺寸变化会改变滚动视口高度：放大后旧偏移可能超过新的 maxScroll，
	// 内容底部会露出空白（关于页表现为日志框内留白）。这里直接收口再摆控件
	const int maxScroll = max(0, CalcPageContentHeight() - ScrollViewportHeight());
	if (m_page_scroll_y > maxScroll)
		m_page_scroll_y = maxScroll;
	UpdateControlsLayout();
	Invalidate();
}
