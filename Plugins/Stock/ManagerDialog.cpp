...
namespace
{
	...
	const wchar_t* kItems_0916_v208[] = {
		L"•  【修复】 分时缓存交易日改用数据自身日期（原先用写入时刻的系统日期，凌晨拉取会把上一交易日数据重复写进今天）；腾讯源补上交易日字段；悬停卡片新增「偏离」行（该分钟价格相对当日均价线的百分比，红高绿低）",
		L"•  【优化】 分时图去掉基金净值（IOPV）紫线；鼠标悬停卡片新增「均价」行（当日累计成交均价，即图上暖金色均价线的数值）",
		L"•  【修复】 修复连续切换股票时新股票K线要等很久才加载：后台焦点任务改为在每个网络请求前检查自身是否已过期，切换股票后旧任务立即退出，不再让新股票排队等它把快照/日K/周K/月K（含超时重试）全部跑完；并去掉切股票时重复执行两次的本地缓存加载",
		L"•  【新增】 悬浮窗新增「BS」交易台账（仅持仓股显示，自选股等其他分组不出现）：筹码峰/盘口/ETF持仓按钮左侧一格，四选互斥；面板按时间升序列出该股全部成交流水（序号/时间/类型/数量/价格），底部固定汇总买卖笔数与股数；双击行弹出暗色弹窗编辑或删除，双击空白处新增，改完即时写库",
		L"•  【优化】 成交录入弹窗再调：全部控件改方角（与设置持仓信息弹窗统一），删除按钮移到弹窗最左、与确定/取消分开避免误按；标签左对齐使左右留白相等、输入框文字加内边距并垂直居中；标题交易所前缀改大写（SH515880）",
		L"•  【优化】 图上 B/S 标记：改用蓝（买）/橙（卖）配色避开K线红绿；外框为正方形且保持原尺寸；引线自影线端点（买接下影线、卖接上影线）向外延伸且长度不小于方块的两倍（原先间距按 RDPI 缩放，在高分屏下算出的引线比方块本身还短，看着仍贴脸）；横向严格对准柱子中心，同日多笔改纵向堆叠",
		L"•  【新增】 日K与分时图新增 B/S 成交标记（同花顺风格：红 B / 绿 S 方块，锚定成交价），做T/减仓的买卖点直接画在K线上，一眼看出成本变化",
